...
In CogStat, variables have three key properties that must be set: **their names**, **their measurement levels**, and, obviously, **the data values** themselves.
...
In CogStat, it is essential to set the measurement levels because automatic data processing relies on this information to determine which analyses to run.

In some software packages, you cannot set the measurement levels, and this information will not be available when you import your data. See below for how CogStat will handle your data when the measurement level is unavailable. If this causes an issue, use any other software that can handle measurement levels to store them with your data.

CogStat handles string variables as nominal, even if another measurement level was set in your data source. So string variables will be set to nominal, regardless of the measurement level provided. Note that if your variable (column) includes even a single string value (i.e., even in a cell), then the whole variable will be considered a string, and the measurement level will be set to nominal.

If the measurement level is not available, it will be set to `unk` (unknown) and, for convenience, treated as an interval variable in most analyses. If this is not your intention, then the measurement level should be set.

## Available file formats

CogStat can import data from several file formats.

- File formats where the measurement level should be set in a separate second row:
...
### How should the data look in your spreadsheet software?
...
* The second row includes the [measurement level of the variables](https://en.wikipedia.org/wiki/Level_of_measurement#Stevens's_typology).
    ...
    * When you display your data in CogStat, a `Type` row (including `num `and `str` types) is shown; these types should *not* be added to the spreadsheet file. The type row will be detected automatically when you import your data.
* All other rows include values of the data.
    * For a missing value, leave the cell empty, or write `nan`. Many other common values will be imported as [missing values](Missing-data).
...
* If you use spreadsheet software, you can copy and paste your data.
    ...
    * The decimal separator should be a dot (i.e., [not comma or other signs](https://en.wikipedia.org/wiki/Decimal_separator#/media/File:DecimalSeparator.svg)). If your spreadsheet software uses a separator other than a dot, change your settings to modify the separator (see how to do it in [Microsoft Office Excel](https://support.office.com/en-us/article/change-the-character-used-to-separate-thousands-or-decimals-c093b545-71cb-4903-b205-aebb9837bd1e) or in [LibreOffice Calc](https://help.libreoffice.org/Common/Languages)).
...
* (New in v2.5) To open the actual data file for editing with the default software for that file type, click on `Data > Open data file with external data editor`. Note that CogStat will use the software that should be used by default to open the given file type. If you want to change the software that handles a file type, change it in your operating settings.
